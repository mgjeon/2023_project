...
model_path = 'model_unet_pi_opt_b3_elr'
...
def criterion(outputs, labels):
    ...
    loss_label = (b-B)
    loss_label = torch.mean(loss_label**2)
    ...
    return loss_label, loss_ff, loss_div
...
import wandb
wandb.init(project='cnn', entity='mgjeon', name='unet_pi_opt_b3_elr')
model.train()
...
for batch_idx, samples in enumerate(dataloaer):
    ...
    loss = loss_label + loss_ff + loss_div
    loss.backward()
    optimizer.step()
    ...
    if (batch_idx+1) % 10 == 0:
        model.eval()
        wandb.log({"loss": loss, "loss_label":loss_label, "loss_ff":loss_ff, "loss_div":loss_div, "learning_rate": scheduler.get_last_lr()[0]})
